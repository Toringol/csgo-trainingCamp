// Code generated by MockGen. DO NOT EDIT.
// Source: ./usecase.go

// Package user is a generated GoMock package.
package user

import (
	reflect "reflect"

	model "github.com/Toringol/csgo-trainingCamp/backend/app/model"
	gomock "github.com/golang/mock/gomock"
)

// MockUsecase is a mock of Usecase interface
type MockUsecase struct {
	ctrl     *gomock.Controller
	recorder *MockUsecaseMockRecorder
}

// MockUsecaseMockRecorder is the mock recorder for MockUsecase
type MockUsecaseMockRecorder struct {
	mock *MockUsecase
}

// NewMockUsecase creates a new mock instance
func NewMockUsecase(ctrl *gomock.Controller) *MockUsecase {
	mock := &MockUsecase{ctrl: ctrl}
	mock.recorder = &MockUsecaseMockRecorder{mock}
	return mock
}

// EXPECT returns an object that allows the caller to indicate expected use
func (m *MockUsecase) EXPECT() *MockUsecaseMockRecorder {
	return m.recorder
}

// SelectUserByID mocks base method
func (m *MockUsecase) SelectUserByID(arg0 int64) (*model.User, error) {
	m.ctrl.T.Helper()
	ret := m.ctrl.Call(m, "SelectUserByID", arg0)
	ret0, _ := ret[0].(*model.User)
	ret1, _ := ret[1].(error)
	return ret0, ret1
}

// SelectUserByID indicates an expected call of SelectUserByID
func (mr *MockUsecaseMockRecorder) SelectUserByID(arg0 interface{}) *gomock.Call {
	mr.mock.ctrl.T.Helper()
	return mr.mock.ctrl.RecordCallWithMethodType(mr.mock, "SelectUserByID", reflect.TypeOf((*MockUsecase)(nil).SelectUserByID), arg0)
}

// SelectUserByUsername mocks base method
func (m *MockUsecase) SelectUserByUsername(arg0 string) (*model.User, error) {
	m.ctrl.T.Helper()
	ret := m.ctrl.Call(m, "SelectUserByUsername", arg0)
	ret0, _ := ret[0].(*model.User)
	ret1, _ := ret[1].(error)
	return ret0, ret1
}

// SelectUserByUsername indicates an expected call of SelectUserByUsername
func (mr *MockUsecaseMockRecorder) SelectUserByUsername(arg0 interface{}) *gomock.Call {
	mr.mock.ctrl.T.Helper()
	return mr.mock.ctrl.RecordCallWithMethodType(mr.mock, "SelectUserByUsername", reflect.TypeOf((*MockUsecase)(nil).SelectUserByUsername), arg0)
}

// CreateUser mocks base method
func (m *MockUsecase) CreateUser(arg0 *model.User) (int64, error) {
	m.ctrl.T.Helper()
	ret := m.ctrl.Call(m, "CreateUser", arg0)
	ret0, _ := ret[0].(int64)
	ret1, _ := ret[1].(error)
	return ret0, ret1
}

// CreateUser indicates an expected call of CreateUser
func (mr *MockUsecaseMockRecorder) CreateUser(arg0 interface{}) *gomock.Call {
	mr.mock.ctrl.T.Helper()
	return mr.mock.ctrl.RecordCallWithMethodType(mr.mock, "CreateUser", reflect.TypeOf((*MockUsecase)(nil).CreateUser), arg0)
}

// UpdateUser mocks base method
func (m *MockUsecase) UpdateUser(arg0 *model.User) (int64, error) {
	m.ctrl.T.Helper()
	ret := m.ctrl.Call(m, "UpdateUser", arg0)
	ret0, _ := ret[0].(int64)
	ret1, _ := ret[1].(error)
	return ret0, ret1
}

// UpdateUser indicates an expected call of UpdateUser
func (mr *MockUsecaseMockRecorder) UpdateUser(arg0 interface{}) *gomock.Call {
	mr.mock.ctrl.T.Helper()
	return mr.mock.ctrl.RecordCallWithMethodType(mr.mock, "UpdateUser", reflect.TypeOf((*MockUsecase)(nil).UpdateUser), arg0)
}

// DeleteUser mocks base method
func (m *MockUsecase) DeleteUser(arg0 int64) (int64, error) {
	m.ctrl.T.Helper()
	ret := m.ctrl.Call(m, "DeleteUser", arg0)
	ret0, _ := ret[0].(int64)
	ret1, _ := ret[1].(error)
	return ret0, ret1
}

// DeleteUser indicates an expected call of DeleteUser
func (mr *MockUsecaseMockRecorder) DeleteUser(arg0 interface{}) *gomock.Call {
	mr.mock.ctrl.T.Helper()
	return mr.mock.ctrl.RecordCallWithMethodType(mr.mock, "DeleteUser", reflect.TypeOf((*MockUsecase)(nil).DeleteUser), arg0)
}

// SelectUserStatsByID mocks base method
func (m *MockUsecase) SelectUserStatsByID(arg0 int64) (*model.UserStats, error) {
	m.ctrl.T.Helper()
	ret := m.ctrl.Call(m, "SelectUserStatsByID", arg0)
	ret0, _ := ret[0].(*model.UserStats)
	ret1, _ := ret[1].(error)
	return ret0, ret1
}

// SelectUserStatsByID indicates an expected call of SelectUserStatsByID
func (mr *MockUsecaseMockRecorder) SelectUserStatsByID(arg0 interface{}) *gomock.Call {
	mr.mock.ctrl.T.Helper()
	return mr.mock.ctrl.RecordCallWithMethodType(mr.mock, "SelectUserStatsByID", reflect.TypeOf((*MockUsecase)(nil).SelectUserStatsByID), arg0)
}

// CreateUserStats mocks base method
func (m *MockUsecase) CreateUserStats(arg0 *model.UserStats) (int64, error) {
	m.ctrl.T.Helper()
	ret := m.ctrl.Call(m, "CreateUserStats", arg0)
	ret0, _ := ret[0].(int64)
	ret1, _ := ret[1].(error)
	return ret0, ret1
}

// CreateUserStats indicates an expected call of CreateUserStats
func (mr *MockUsecaseMockRecorder) CreateUserStats(arg0 interface{}) *gomock.Call {
	mr.mock.ctrl.T.Helper()
	return mr.mock.ctrl.RecordCallWithMethodType(mr.mock, "CreateUserStats", reflect.TypeOf((*MockUsecase)(nil).CreateUserStats), arg0)
}

// UpdateUserStats mocks base method
func (m *MockUsecase) UpdateUserStats(arg0 *model.UserStats) (int64, error) {
	m.ctrl.T.Helper()
	ret := m.ctrl.Call(m, "UpdateUserStats", arg0)
	ret0, _ := ret[0].(int64)
	ret1, _ := ret[1].(error)
	return ret0, ret1
}

// UpdateUserStats indicates an expected call of UpdateUserStats
func (mr *MockUsecaseMockRecorder) UpdateUserStats(arg0 interface{}) *gomock.Call {
	mr.mock.ctrl.T.Helper()
	return mr.mock.ctrl.RecordCallWithMethodType(mr.mock, "UpdateUserStats", reflect.TypeOf((*MockUsecase)(nil).UpdateUserStats), arg0)
}

// DeleteUserStats mocks base method
func (m *MockUsecase) DeleteUserStats(arg0 int64) (int64, error) {
	m.ctrl.T.Helper()
	ret := m.ctrl.Call(m, "DeleteUserStats", arg0)
	ret0, _ := ret[0].(int64)
	ret1, _ := ret[1].(error)
	return ret0, ret1
}

// DeleteUserStats indicates an expected call of DeleteUserStats
func (mr *MockUsecaseMockRecorder) DeleteUserStats(arg0 interface{}) *gomock.Call {
	mr.mock.ctrl.T.Helper()
	return mr.mock.ctrl.RecordCallWithMethodType(mr.mock, "DeleteUserStats", reflect.TypeOf((*MockUsecase)(nil).DeleteUserStats), arg0)
}
